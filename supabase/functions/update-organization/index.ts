...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: user, error: userError } = await supabase.from("azure_users").select("id, role, organization_id").eq("email", userEmail).eq("organization_id", organizationId).eq("role", "admin").single();
    if (userError || !user) {
      return new Response(JSON.stringify({
        success: false,
    ...
    // Update organization
    const { data: updatedOrganization, error: updateError } = await supabase.from("organizations").update({
