...
const ResetApprovalClient = () => {
  ...
  const handleResetRequest = async (e: React.FormEvent) => {
    ...
    setResetRequest({
      email,
      status: 'loading',
      message: 'Sending approval request to user...'
    });
    ...
    try {
      ...
      // Simulate sending approval request (in production, use actual Microsoft Graph API)
      console.log('Sending approval request to:', email);
      console.log('Using access token:', tokenResponse.accessToken);
      
      // Simulate API success - in production this would call Microsoft Graph API
      // to send an approval notification to the user
      setTimeout(() => {
      ...
      case 'approved':
        return (
          <div className="mt-6 p-4 border rounded-md bg-green-50 flex flex-col items-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">Processing account changes...</p>
  ...
  return (
    <Card className="w-full max-w-md security-card">
      <CardHeader>
        <CardTitle className="text-center text-2xl">Admin Approval Request</CardTitle>
        <CardDescription className="text-center">
          Enter the user's email to request approval
        </CardDescription>
      </CardHeader>
      <CardContent>
      ...
      <CardFooter className="flex justify-center text-sm text-muted-foreground">
        {resetRequest.status === 'idle' && "User will receive a notification to approve"}
      </CardFooter>
...
export default ResetApprovalClient;
